...
function generateKconfigBoard(boardName, mcu, supportsNS) {
  const boardNameUpper = boardName.toUpperCase();
  const mcuUpper = mcu.toUpperCase();
...
function generateMainDts(boardName, mcu) {
...
for (const mcu of manifest.mcus) {
  ...
  if (uart) {
    ...
    console.log(`  UART pins: ${JSON.stringify(uart.pinFunctions)}`);
  } else {
  ...
  files[`${boardName}_${mcuId}_cpuapp.dts`] = generateMainDts(boardName, mcuId);
